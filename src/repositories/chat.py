...
class ChatRepository:
    ...
    def create(self, data: ChatSchema) -> ChatSchema:
        try:
    ...
    def read(self, _id: int) -> ChatTable:
        ...
        return ChatSchema.model_validate(db_record) if db_record else None

    def update(self, _id: int, data: ChatSchema) -> ChatSchema:
        db_record = self.db_session.query(ChatTable).filter(ChatTable.id == _id).first()
        if db_record:
        ...
        return ChatSchema.model_validate(db_record) if db_record else data
