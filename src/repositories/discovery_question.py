...
from src.schemas import DiscoveryQuestion
...
class DiscoveryQuestionRepository:
    ...
    def create(self, data: DiscoveryQuestion) -> DiscoveryQuestion:
        try:
    ...
    def read(self, _id: int) -> ChatTable:
        ...
        return DiscoveryQuestion.model_validate(db_record) if db_record else None

    def update(self, _id: int, data: DiscoveryQuestion) -> DiscoveryQuestion:
        db_record = self.db_session.query(ChatTable).filter(ChatTable.id == _id).first()
        if db_record:
        ...
        return DiscoveryQuestion.model_validate(db_record) if db_record else data
